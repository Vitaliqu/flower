...
const FullPage = observer(() => {
  ...
  const [currentFlower, setFlower] = useState(
    flower.flowers.length > 0
      ? flower.flowers.find((element) => element.id === parseInt(params.id))
      : {},
  );
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      await setFlower(
        flowers.rows.find((element) => element.id === parseInt(params.id)),
      );
      if (!flowers.rows.map((element) => `${element.id}`).includes(params.id))
  ...
  const handleLikeClick = () => {
    if (!hearsState) {
      ...
      flower.setLiked(
        flower.liked.filter((element) => element !== currentFlower.id),
      );
      setHeart(false);
      localStorage.setItem("liked", JSON.stringify(flower.liked));
    }
      ...
      <div className={styles.flowerCard}>
        ...
        <div className={styles.info}>
          <div className={styles.nameWrapper}>
            <p className={styles.flowerName}>{currentFlower.name}</p>
            <img
              onClick={handleLikeClick}
              className={styles.heart}
              src={hearsState ? activeHeart : heart}
              alt=""
            />
          </div>
          ...
          <div className={styles.descriptionWrapper}>
            <div
              ...
              dangerouslySetInnerHTML={{ __html: currentFlower.description }}
            ></div>
  ...
  const renderTabletCard = () => {
    ...
    return (
      <div className={styles.tabletFlowerCard}>
  ...
  return isLoading ? (
    ...
    <>
      ...
      <div className={styles.container}>
        {isTablet ? renderTabletCard() : renderCard()}
      </div>
    </>
  );
});
